...
int loadBMP(char *filename, textureImage *texture)
{
	...
	short int test;
    for (i = 0; i < (14 + 40) / 2; i++) {
        fread(&test, sizeof(short int), 1, file);
        printf("%0x ", test);
    }
    ...
    printf("Width of Bitmap: %0x\n", texture->width);
    /* get the height of the bitmap */
    fread(&texture->height, sizeof(int), 1, file);
    printf("Height of Bitmap: %0x\n", texture->height);
...
int main(int argc, char **argv)
{
    ...
    GLWin.fs = True;
    if (!createGLWindow("NeHe's Display List Tutorial", 1600, 900, 24,
        GLWin.fs))
    {
        done = True;
    }
